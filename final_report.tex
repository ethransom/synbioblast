...
results of these queries. A prototype has shown that this approach does not pose
availability constraints on SynBioHub's backing database, or is unfeasible to run 
...
fashion. An alternate architecture is discussed in the Future Work section.
...
of the FASTA directory and run the \texttt{cat} command for each file. Figure <x> shows 
a command similar to the one used to build the database.
...
find -name '*.fasta' -exec cat {} \; 
  | ./makeblastdb -in -
\end{lstlisting}
\caption{Find is used to concatenate the FASTA files and pipe them to \texttt{makeblastdb}}
\label{fig_sim}
\end{figure}
...
BLAST implementation. A webserver displays a form to the user. Users input their query 
...
\begin{itemize}
  \item 
    As discussed above, modifing the scraping code to append new sequences to 
